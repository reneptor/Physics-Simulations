...
constexpr float time_span = 10; // Time in seconds 
...
struct ParticleSimulation{
    ...
    void RunSimulation(
        std::array<float, T>& logged_temperatures,
        ...
        IterTimestep(0, logged_temperatures, logged_total_energies, logged_particle_energies, logged_positions);
        ...
        for (int t = 1; t < T; t++){
            IterTimestep(t, logged_temperatures, logged_total_energies, logged_particle_energies, logged_positions);
        }
    ...
    void IterTimestep(int t, 
        ...
        logged_temperatures[t] = TempInstantaneous();
...
void WritePositionsToCSV(const std::string& filename, 
    const std::array<float, T> logged_temperatures, 
    const std::array<std::tuple<float, float, float>, T>& logged_total_energies,
...
int main(int argc, char *argv[]){
    ...
    sim.RunSimulation(logged_temperatures, logged_total_energies, logged_particle_energies, logged_positions);
    std::cout << "Simulation complete.\n";

    WritePositionsToCSV(filename, logged_temperatures, logged_total_energies, logged_particle_energies, logged_positions);
